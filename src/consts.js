export const msInMin = 60 * 1000;
export const FILTERS = [`All movies`, `Watchlist`, `History`, `Favorites`];
export const DIRECTORS = [
  `Quentin Tarantino`,
  `Christopher Nolan`,
  `Denis Villeneuve`,
  `Guy Ritchie`,
  `Darren Aronofsky`,
  `Wes Anderson`,
  `Taika Waititi`,
  `David Fincher`
];
export const WRITERS = [
  `Jonathan Nolan`,
  `Christopher Nolan`,
  `Quentin Tarantino`,
  `Hampton Fancher`,
  `Guy Ritchie`,
  `Darren Aronofsky`,
  `Wes Anderson`,
  `Chuck Palahniuk`
];
export const ACTORS = [
  `Brad Pitt`,
  `Leonardo DiCaprio`,
  `Matthew McConaughey`,
  `Jessica Chastain`,
  `Matt Diamond`,
  `Edward Norton`,
  `Helena Bonham Carter`,
  `Margot Robbie`,
  `Ryan Gosling`,
  `Anne Hathaway`,
  `Jennifer Lawrence`
];
export const COUNTRIES = [
  `USA`,
  `UK`,
  `Russia`,
  `France`,
  `Germany`,
  `Australia`
];
export const GENRES = [
  `Action`,
  `Horror`,
  `Comedy`,
  `Drama`,
  `Detective`,
  `Romance`
];
export const AGE_RATINGS = [
  0,
  3,
  12,
  16,
  18
];
export const COMMENTATOR_NAMES = [
  `Max`,
  `Ivan`,
  `Andrew`,
  `Daniel`,
  `Denis`,
  `Alex`
];
export const FILM_TITLES_TO_ORIGINAL_TITLES = new Map([
  [`Интерстеллар`, `Interstellar`],
  [`Однажды... в Голивуде`, `Once Upon a Time... in Hollywood`],
  [`Омерзительная восмерка`, `The Hateful Eight`],
  [`Бегущий по лезвию 2049`, `Blade Runner 2049`],
  [`Карты, деньги, два ствола`, `Lock, Stock and Two Smoking Barrels`],
  [`мама!`, `mother!`],
  [`Остров собак`, `Isle of Dogs`],
  [`Кролик Джоджо`, `Jojo Rabbit`],
  [`Большой Лебовски`, `The Big Lebowski`],
  [`Темный рыцарь`, `The Dark Knight`],
  [`Прибытие поезда на вокзал Ла-Сиоты`, `L' Arrivée d'un train à la Ciotat`],
  [`Бойцовский клуб`, `Fight Club`]
]);
export const FILM_POSTERS = [
  `made-for-each-other.png`,
  `popeye-meets-sinbad.png`,
  `sagebrush-trail.jpg`,
  `santa-claus-conquers-the-martians.jpg`,
  `the-dance-of-life.jpg`,
  `the-great-flamarion.jpg`,
  `the-man-with-the-golden-arm.jpg`
];
export const DESCRIPTION_SENTENCES = [
  `Lorem ipsum dolor sit amet, consectetur adipiscing elit.`,
  `Cras aliquet varius magna, non porta ligula feugiat eget.`,
  `Fusce tristique felis at fermentum pharetra.`,
  `Aliquam id orci ut lectus varius viverra.`,
  `Nullam nunc ex, convallis sed finibus eget, sollicitudin eget ante.`,
  `Phasellus eros mauris, condimentum sed nibh vitae, sodales efficitur ipsum.`,
  `Sed blandit, eros vel aliquam faucibus, purus ex euismod diam, eu luctus nunc ante ut dui.`,
  `Sed sed nisi sed augue convallis suscipit in sed felis.`,
  `Aliquam erat volutpat.`,
  `Nunc fermentum tortor ac porta dapibus.`,
  `In rutrum ac purus sit amet tempus.`
];
export const EMOTIONS = [`smile`, `sleeping`, `puke`, `angry`];
export const KEY = {
  ENTER: `Enter`,
  ESC: `Escape`
};
